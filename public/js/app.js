!function(n) {
    "use strict";
    function s() {
        for (var e = document.getElementById("topnav-menu-content").getElementsByTagName("a"), t = 0, nLen = e.length; t < nLen; t++)
    ...
    n("#vertical-menu-btn").on("click", function(e) {
        ...
        if (992 <= n(window).width()) {
            n("body").toggleClass("vertical-collpsed");

            // Kondisi untuk toggle class d-flex sesuai vertical-collpsed
            if (n("body").hasClass("vertical-collpsed")) {
                n("#sidebar-menu").removeClass("d-flex");
            } else {
                n("#sidebar-menu").addClass("d-flex");
            }
    ...
    n(document).ready(function() {
        var e;
        0 < n("#sidebar-menu").length && 0 < n("#sidebar-menu .mm-active .active").length && (
        ...
        // Inisialisasi DataTable di sini, misal untuk tabel dengan id #datatable
        if (n.fn.DataTable) {
            n('#datatable').DataTable();
        }
    }),
    ...
    window.sessionStorage && ((a = sessionStorage.getItem("is_visited")) ? (
